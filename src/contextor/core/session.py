"""SessionPersistence — сохранение и восстановление сессий Contextor.
...
class SessionPersistence:
    ...
    def load(
        ...
        try:
            ...
            meta = json.loads(self._meta_path.read_text(encoding='utf-8', errors='replace'))
            result["meta"] = meta
    ...
    def _load_chat_history(self) -> list[dict]:
        ...
        if not self._history_path.exists():
            return []
        try:
            content = self._history_path.read_text(encoding='utf-8', errors='replace').strip()
            if not content:
                return []
