"""MemoryStorage — долгосрочное хранилище фактов Contextor.
...
# ── Embedding provider выбор из config.yaml ──────────────────────────────────
_embed_provider: str | None = None   # 'sentence_transformers' | 'ollama' | None


def _init_embed_config() -> None:
    """Читает embedder.provider и embedder.model из config.yaml один раз.
    Это позволяет пользователю выбирать провайдер через конфигурацию,
    а не через hardcode. По умолчанию — пробуем sentence_transformers, затем ollama.
    """
    global _embed_provider, OLLAMA_EMBED_MODEL
    if _embed_provider is not None:
        return
    try:
        import yaml as _yaml, pathlib as _pl
        # Ищем config.yaml рядом с пакетом или в рабочей директории
        _candidates = [
            _pl.Path("config.yaml"),
            _pl.Path(__file__).parent.parent.parent.parent.parent / "config.yaml",
            _pl.Path(__file__).parent.parent.parent.parent / "config.yaml",
        ]
        for _c in _candidates:
            if _c.exists():
                _cfg = _yaml.safe_load(_c.read_text(encoding='utf-8', errors='replace')) or {}
                _emb = _cfg.get("models", {}).get("embedder", {})
                _embed_provider = _emb.get("provider", "auto")
                _model = _emb.get("model", "")
                if _model:
                    OLLAMA_EMBED_MODEL = _model
                logger.info(f"[Storage] Embedding provider: {_embed_provider}, model: {OLLAMA_EMBED_MODEL}")
                return
    except Exception as _e:
        logger.debug(f"[Storage] config.yaml read failed: {_e}")
    _embed_provider = "auto"  # default: пробуем sentence_transformers, затем ollama
...
def _get_st_model():
    ...
    # Инициализируем конфиг провайдера (один раз)
    _init_embed_config()

    # Если пользователь выбрал Ollama — не пробуем sentence_transformers
...
def _get_ollama_embedding(text: str) -> list[float] | None:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=EMBED_TIMEOUT) as resp:
            ...
            # Новый Ollama API: {"embeddings": [[...]]} или {"embedding": [...]}
            raw = data.get("embeddings") or data.get("embedding") or []
            if raw and isinstance(raw[0], list):
                embedding = raw[0]  # [[...]] формат
            else:
                embedding = raw     # [...] формат
