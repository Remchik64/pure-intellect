...
def _find_config_yaml() -> Path | None:
    ...
    if appdata:
        candidates.append(Path(appdata) / "Contextor" / "config.yaml")

    # 3. Linux/macOS XDG
    candidates.append(Path.home() / ".config" / "contextor" / "config.yaml")
    ...
    # 6. System-wide
    candidates.append(Path("/etc/contextor/config.yaml"))
...
def save_model_to_config(role: str, model_name: str) -> bool:
    ...
    if config_path is None:
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            config_dir = Path(appdata) / "Contextor"
        else:
            config_dir = Path.home() / ".config" / "contextor"
